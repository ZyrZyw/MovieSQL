...
def getmovielist(request):
    ...
    #return render_to_response("movie/allfilms.html", locals())
...
def getdetailinfo(request, film_id):
    cast_list = [x.name for x in CastInfo.objects.filter(film_id=film_id)]
    director_list = [x.name for x in DirectorFilmInfo.objects.filter(film_id=film_id)]
    film = FilmInfo.objects.filter(id=film_id)
    context = {
        'cast_list': cast_list,
...
def searchmovie(request):
    random_num = random.randint(0,99)
    imdbmovie_list = FilmInfo.objects.order_by('rate')[random_num:random_num+6]
    ...
    if 'q' in request.GET:
        ...
        if len(querystring)==0:
            ...
            movielist = FilmInfo.objects.filter(film_name__contains=querystring)
